...
- **체크리스트**: 아이별로 오늘의 할 일을 시간대(아침🌅/점심☀️/저녁🌙/언제든✨)별로 묶어서 보여줘요. 모든 일과를 다 하면 ⭐ 별 1개 자동 지급 + 컨페티 축하 🎉
- **선물 상점**: 모은 별로 등록된 선물을 구매 (재고 관리 지원)
- **통계 모드**: 이번 주 목표(7일 완벽 달성) 진행 현황, 최근 14일 달성률 그래프, 연속 완료일(스트릭), 완벽한 날 수, 일과별 누적 현황
- **주간 목표 보너스**: 한 주(월~일) 동안 매일 모든 일과를 완벽하게 끝내면 추가 보너스 별을 받아요 (개수는 부모님 모드에서 설정 가능)
- **부모님 모드** (비밀번호로 진입)
  - 아이 등록 / 수정 / 삭제 (이름, 아바타, 색상)
  - 아이에게 별 직접 선물하기 (이유 메모 가능)
  - 일과 등록 / 수정 / 삭제 (시간대, 전체 공통 또는 특정 아이 전용 설정 가능)
  - 주간 목표 보너스 별 개수 설정
